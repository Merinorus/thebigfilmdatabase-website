...
const processFrame = function () {
    ...
    if (code.format) {
        ...
        if (fullCode !== lastDetectedCode) {
            ...
            drawResult(code)
            if (code.format === "ITF") {
                window.location.href = "{{ url_for('search') }}?dx_full=" + encodeURIComponent(code.text);
            } else if (code.format === "DXFilmEdge") {
                let dx_full = escapeTags(code.text);
                window.location.href = "{{ url_for('search') }}?dx_number=" + encodeURIComponent(code.text);
            }
        }
    } else {
